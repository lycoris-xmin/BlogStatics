$(async function () {
  ...
  const makeTargetGlueColor = target => {
    ...
    // return `rgba(170,221,255,$selector {alpha.toFixed(3)})`;
  ...
  const getHighScore = async () => {
    const result = await db.getData(highScoreKey);
    let source = 0;
    if (result) {
      source = parseInt(result.source, 10);
    } else {
      await db.create({
        id: highScoreKey,
        source: 0
  ...
  let _lastHighscore = await getHighScore();

  const setHighScore = async score => {
    _lastHighscore = await getHighScore();
    await db.update({ id: highScoreKey, source: score });
  };
  ...
  const colorToHex = color => {
    return '#' + (color.r | 0).toString(16).padStart(2, '0') + (color.g | 0).toString(16).padStart(2, '0') + (color.b | 0).toString(16).padStart(2, '0');
  };
  ...
  const shadeColor = (color, lightness) => {
    ...
    return '#' + (lerp(color.r, other, mix) | 0).toString(16).padStart(2, '0') + (lerp(color.g, other, mix) | 0).toString(16).padStart(2, '0') + (lerp(color.b, other, mix) | 0).toString(16).padStart(2, '0');
  };
  ...
  function makeRecursiveCubeModel({ recursionLevel, splitFn, color, scale = 1 }) {
    ...
    cubeOrigins.forEach((origin, cubeIndex) => {
      ...
      const occlusion = Math.max(Math.abs(origin.x), Math.abs(origin.y), Math.abs(origin.z)) / maxComponent;
  ...
  function optimizeModel(model, threshold = 0.0001) {
    ...
    const compareVertices = (v1, v2) => Math.abs(v1.x - v2.x) < threshold && Math.abs(v1.y - v2.y) < threshold && Math.abs(v1.z - v2.z) < threshold;
  ...
  class Entity {
    ...
    transform() {
      transformVertices(this.model.vertices, this.vertices, this.x, this.y, this.z, this.rotateX, this.rotateY, this.rotateZ, this.scaleX, this.scaleY, this.scaleZ);
  ...
  const hudContainerNode = $selector('.hud');
  ...
  // Score //
  ///////////
  const scoreNode = $selector('.score-lbl');
  const cubeCountNode = $selector('.cube-count-lbl');
  ...
  function renderScoreHud() {
    ...
    cubeCountNode.innerText = `切中次数: ${state.game.cubeCount}`;
  }
  ...
  handlePointerDown($selector('.pause-btn'), () => pauseGame());

  ////////////////////
  // Slow-Mo Status //
  ////////////////////

  const slowmoNode = $selector('.slowmo');
  const slowmoBarNode = $selector('.slowmo__bar');

  function renderSlowmoStatus(percentRemaining) {
    slowmoNode.style.opacity = percentRemaining === 0 ? 0 : 1;
    slowmoBarNode.style.transform = `scaleX($selector {percentRemaining.toFixed(3)})`;
  }
  ...
  const menuContainerNode = $selector('.menus');
  const menuMainNode = $selector('.menu--main');
  const menuPauseNode = $selector('.menu--pause');
  const menuScoreNode = $selector('.menu--score');

  const finalScoreLblNode = $selector('.final-score-lbl');
  const highScoreLblNode = $selector('.high-score-lbl');
  ...
  function renderMenus() {
    ...
    switch (state.menus.active) {
      ...
      case MENU_SCORE:
        ...
        if (isNewHighScore()) {
          highScoreLblNode.textContent = '创造了一个新纪录!';
        } else {
          getHighScore().then(result => {
            highScoreLblNode.textContent = `最高分数: ${formatNumber(result)}`;
          });
        }
  ...
  // Main Menu
  handleClick($selector('.play-normal-btn'), () => {
    setGameMode(GAME_MODE_RANKED);
    setActiveMenu(null);
    resetGame();
  });

  handleClick($selector('.play-casual-btn'), () => {
  ...
  // Pause Menu
  handleClick($selector('.resume-btn'), () => resumeGame());
  handleClick($selector('.menu-btn--pause'), () => setActiveMenu(MENU_MAIN));

  // Score Menu
  handleClick($selector('.play-again-btn'), () => {
    setActiveMenu(null);
    resetGame();
  });

  handleClick($selector('.menu-btn--score'), () => setActiveMenu(MENU_MAIN));
  ...
  function draw(ctx, width, height, viewScale) {
    ...
    allPolys.forEach(p => {
      ...
      if (!p.wireframe) {
        ...
        const lightness = normalLight > 0 ? 0.1 : ((normalLight ** 32 - normalLight) / 2) * 0.9 + 0.1;
  ...
  function setupCanvases() {
    ...
    function handleResize() {
      const w = $selector('div.container').clientWidth;
      const h = $selector('div.container').clientHeight;
      viewScale = h / 1000;
      width = w / viewScale;
      height = h / viewScale;
      canvas.width = w * dpr;
