...
(function ($) {
  ...
  defaultLayoutTemplates = {
     ...
     */
    default:
      '<div class="mt-wrapper {mainClass}" style="height: 100%;" >' +
  ...
  MultiTabs.prototype = {
    ...
    _getTabPaneHtml: function (param) {
      ...
      if (!param.content && param.iframe) {
        return defaultLayoutTemplates.iframeTabPane.replace('{class}', options.content.iframe.class).replace('{tabPaneId}', param.did);
      } else {
        return defaultLayoutTemplates.ajaxTabPane.replace('{class}', options.content.ajax.class).replace('{tabPaneId}', param.did).replace('{content}', param.content);
      }
    },
    ...
    _getNavTabHtml: function (param) {
      ...
      return defaultLayoutTemplates.navTab.replace('{index}', param.index).replace('{navTabId}', param.did).replace('{url}', param.url).replace('{title}', param.title).replace('{type}', param.type) + closeBtnHtml;
    },
    ...
    close: function (navTab) {
      ...
      if ($navTabLi.length && $tabPane.length && $tabPane.hasClass('unsave') && !self._unsaveConfirm()) {
        return self;
      }
    ...
    _init: function (options) {
      var self = this,
        $el = self.$element;
      $el.html(
        defaultLayoutTemplates[options.nav.layout]
    ...
    _listen: function () {
      ...
      handler($(document), 'click', options.selector, function () {
        ...
        if (!$(this).parent().parent('ul').hasClass('dropdown-menu')) {
          // 20190402改，下拉菜单中的网址采用data-url，并且不阻止后面的动作
      ...
      if (options.nav.layout === 'classic') {
        handler($el.nav, 'click', '.mt-dropdown:not(.open)', function () {
          //just trigger when dropdown not open.
    ...
    _fixTabPosition: function (navTab) {
      ...
      if (sumTabsWidth < navPanelWidth) {
        ...
        if (prevWidth + tabWidth + sumNextWidth <= navPanelWidth) {
          ...
          //nav-panel can not contain the navTab and his right tabs
    ...
    _getHiddenList: function () {
      ...
      if (sumTabsWidth < navPanelWidth) {
        ...
        if (sumTabsWidth > marginLeft) {
          //check if the right part have hidden tabs
