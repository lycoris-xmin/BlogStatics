    ...
    this.init = function () {
      if ($this.children('.' + defaultClass).length > 0) {
        ...
        var $maskHtml = $('<div />').addClass(defaultClass),
          ...
          $spinnerHtml = opts.imgUrl
            ? $('<img />').attr('src', opts.imgUrl).addClass(defaultClass)
            : $('<div />').addClass('spinner-border').addClass(defaultClass).addClass(opts.spinnerColorClass).css(this.getSpinnerSize());
        ...
        $(window)
          .off('resize.' + defaultClass)
          .on('resize.' + defaultClass, function () {
            toolMethods.resizeStyle();
          });
      }
    };
